...
const NewsPost = () => {
  ...
  const post: PostData | null = postData ? {
    id: postData?.id,
  ...
  const relatedPosts = relatedData?.data?.map((p: Record<string, unknown>) => ({
    id: p?.id as number,
    ...
    featuredImage: p?.featuredImage as StrapiMedia,
    imageUrl: p?.imageUrl as string,
    category: p?.category as { id?: number; name?: string; slug?: string } | null,
  })) || [];
